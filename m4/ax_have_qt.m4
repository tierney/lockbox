  ...
  if test x"$with_Qt_dir" = x"no" ||
    ...
    if test x"$with_Qt_dir" != x; then
      ...
      if test x"$ax_qt_lib" = x; then
        ax_qt_lib="`ls $ax_qt_lib_dir/libQt5* | sed -n 1p |
...
[
  ...
  if (test -x $ax_possible_qt_dir/bin/moc) &&
     ((ls $ax_possible_qt_dir/lib/libQt5* > /dev/null 2>/dev/null) ||
      (ls $ax_possible_qt_dir/lib64/libQt5* > /dev/null 2>/dev/null)); then
        ...
        [
	      ...
	      /opt/lib64
            ...
            for ax_dir in $ax_dir_list; do
              if ls $ax_dir/libQt5* >/dev/null 2>/dev/null; then
                # Gamble that it's the first one...
                ax_qt_lib="`ls $ax_dir/libQt5* | sed -n 1p |
